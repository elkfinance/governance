...
contract TokenVester is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        require(
            ...
            "TokenVester::constructor: Vesting amount too high"
        );
        require(
            halvingPeriod_ >= 1,
            "TokenVester::constructor: Invalid halving period"
        );
        ...
        lastUpdate = 1649505600; // April 9, 2022 at 12pm GMT
    ...
    function startVesting() external onlyOwner {
        require(
            !vestingEnabled,
            "TokenVester::startVesting: vesting already started"
        );
        require(
            elk.balanceOf(address(this)) >= startingBalance,
            "TokenVester::startVesting: incorrect ELK supply"
        );
        require(
            recipient != address(0),
            "TokenVester::startVesting: recipient not set"
        );
    ...
    function setRecipient(address recipient_) external onlyOwner {
        require(
            !vestingEnabled,
            "TokenVester::setRecipient: vesting already started"
        );
    ...
    function claim() external nonReentrant returns (uint256) {
        ...
        require(
            ...
            "TokenVester::claim: only recipient can claim"
        );
    ...
    function claimAll() external nonReentrant returns (uint256) {
        ...
        require(
            msg.sender == recipient,
            "TokenVester::claimAll: only recipient can claim"
        );
    ...
    function _claim() private returns (uint256) {
        require(
            ...
            "TokenVester::_claim: not time yet"
        );
